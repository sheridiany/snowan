...
def render_diagram(svg: str, title: str = "") -> str:
    ...
    - 方框用 class `box`;上色用 `c-blue / c-teal / c-amber / c-green / c-red / c-purple / c-gray`(放在形状或它的直接父 <g> 上,别隔层)。默认所有节点同色(c-blue);只有两类本质不同的东西/流向才加第二种强调色 + 一行图例。≤2 色,**绝不每个盒子一种颜色(彩虹)**。
