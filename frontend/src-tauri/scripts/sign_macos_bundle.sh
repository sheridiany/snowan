...
#
# PyInstaller collects the Python runtime and native extension libraries from
# third-party packages (onnxruntime, numpy, etc.). Re-signing every Mach-O file
# after collection keeps the backend executable, Python runtime, and native
# dependencies in one signature state before Tauri embeds them in the final app.
